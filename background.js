...
chrome[roe].onMessage.addListener(
    function(request, sender, sendResponse) {
        ...
        //downloadTextFile(request.timing.connectEnd - request.timing.connectStart);
        downloadTextFile(request.time, request.timing.connectEnd - request.timing.connectStart)
    }
);
...
var downloadTextFile = function(loadtime, conn_time) {
    if(!loadtime) {
        loadtime = '0';
    }
    if (!conn_time) {
    ...
    var file = new File([loadtime, ' ', conn_time], "loadtime.txt", { type: "text/plain;charset=utf-8" });
    saveAs(file);
}
